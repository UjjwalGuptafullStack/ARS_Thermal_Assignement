...
\TODO{Interpret the class table briefly. It does two jobs: it confirms
built-up genuinely drives the urban signal rather than the urban zone
merely containing warm pixels, and it quantifies what including water
or mangrove in a rural reference would have cost. Link the second
point to Table~\ref{tab:landsat_water}.}
...
\TODO{Interpret. The sign reversal between day and night is the same
one predicted in Section~\ref{sec:waterexcl}, but the magnitude here
dwarfs the MODIS result. Why would a single-date \SI{30}{\metre} scene
be so much more sensitive to including the sea than a multi-year
\SI{1}{\kilo\metre} seasonal composite? Consider what averaging over
five years does to a water surface, and what the different pixel sizes
do to the land--sea boundary.}
